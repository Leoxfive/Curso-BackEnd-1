...
router.post("/", async (req, res) => {
  try {
    ...
    res
      ...
      .json({ error: err.message || "Error al crear el producto" });
  }
});
router.put("/:pid", async (req, res) => {
  try {
    ...
    res
      .status(err.status || 500)
      .json({ error: err.message || "Error al actualizar el producto" });
  }
});
router.delete("/:pid", async (req, res) => {
  try {
    ...
    res
      .status(err.status || 500)
      .json({ error: err.message || "Error al eliminar el producto" });
  }
});
